...
export default function Home() {
  ...
  return (
    <main className="mx-auto grid min-h-screen w-full max-w-[1600px] grid-cols-1 gap-4 p-3 text-zinc-100 sm:p-4 lg:h-screen lg:grid-cols-[minmax(0,1fr)_380px] xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="flex min-h-[72vh] flex-col rounded-3xl border border-zinc-800 bg-zinc-950 p-4 shadow-2xl shadow-black/30 lg:min-h-0">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-zinc-800 pb-3">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-zinc-500">Party chat</p>
            <h1 className="mt-2 text-2xl font-semibold">{zoneLabel[party.zone]}</h1>
          </div>
          <button className="rounded-xl border border-zinc-700 px-3 py-2 text-xs text-zinc-300 hover:bg-zinc-900" onClick={() => navigator.clipboard.writeText(window.location.href)}>Copy invite link</button>
        </div>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto py-4 pr-1">
          {party.messages.length === 0 ? <p className="rounded-2xl border border-dashed border-zinc-800 p-4 text-sm text-zinc-500">Join the party and write the first action. The gamemaster will answer after each player message.</p> : null}
          {party.messages.map((message) => <article key={message.id} className="rounded-2xl bg-zinc-900 p-3 text-sm md:text-base"><p className="mb-1 text-xs uppercase tracking-wider text-zinc-500">{message.playerName ?? message.sender}</p><p className="whitespace-pre-wrap leading-relaxed">{message.text}</p></article>)}
          {isGmThinking ? <p className="text-sm text-zinc-500">Gamemaster is thinking…</p> : null}
        </div>
        <div className="flex gap-2 border-t border-zinc-800 pt-3"><textarea value={draft} onChange={(event) => setDraft(event.target.value)} disabled={!player} placeholder={player ? "What do you do?" : "Join to chat"} className="min-h-16 flex-1 resize-none rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2 outline-none" /><button onClick={sendMessage} disabled={!player || !draft.trim()} className="rounded-xl bg-zinc-100 px-4 py-2 text-zinc-950 disabled:opacity-40">Send</button></div>
      </section>

      <aside className="rounded-3xl border border-zinc-800 bg-zinc-950 p-5 shadow-2xl shadow-black/30 lg:overflow-y-auto">
        <p className="text-sm uppercase tracking-[0.3em] text-zinc-500">Current zone</p>
        <h2 className="mt-3 text-3xl font-semibold">{zoneLabel[party.zone]}</h2>
        <p className="mt-4 text-sm leading-6 text-zinc-400">A plain shared room for the party. Use chat to act, then ready up when everyone wants to switch zones.</p>
        <div className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
          <div className="flex items-center justify-between gap-3"><h3 className="font-medium">Party readiness</h3><span className="text-sm text-zinc-500">{players.length}/{maxPlayers} players</span></div>
          <div className="mt-4 grid gap-2">
            {players.length === 0 ? <p className="rounded-xl bg-zinc-950 px-3 py-2 text-sm text-zinc-500">No players have joined yet.</p> : null}
